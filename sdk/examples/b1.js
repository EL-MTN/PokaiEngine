	...
	bot.setEventHandlers({
		...
		onTurnStart: async (data) => {
			...
			try {
				...
				console.log(`[B1] 📊 Hand: ${formatCards(gameState.playerCards)} | Pot: ${gameState.potSize} | Phase: ${gameState.currentPhase}`);
				
				// Get possible actions
				const actions = await bot.getPossibleActions();
				console.log(`[B1] 🎲 Available: ${actions.map(a => a.type).join(', ')}`);
