...
import { GamePhase, Position, Rank, Suit } from '@/domain/types';
...
describe('GameState', () => {
	...
	describe('Player Management', () => {
		...
		it('should throw error when adding duplicate player', () => {
			...
			gameState.addPlayer(player1);
			expect(() => gameState.addPlayer(player2)).toThrow(
		...
		it('should throw error when removing non-existent player', () => {
			expect(() => gameState.removePlayer('nonexistent')).toThrow(
				'Player not found',
			);
		});
	...
	describe('Hand Lifecycle', () => {
		...
		it('should throw error when trying to advance from HandComplete', () => {
			...
			expect(() => gameState.advancePhase()).toThrow(
	...
	describe('Betting Mechanics', () => {
		...
		it('should throw error for non-existent player bet', () => {
			expect(() => gameState.processBet('nonexistent', 100)).toThrow(
				'Player not found',
			);
		});

		it('should throw error for non-existent player blind', () => {
			expect(() => gameState.processBlind('nonexistent', 5)).toThrow(
				'Player not found',
			);
		});
