...
describe('Hole Card Visibility System', () => {
	...
	describe('shouldShowHoleCards function', () => {
		it('should show own cards to players', () => {
			expect(shouldShowHoleCards('player', 'p1', 'p1', false, false)).toBe(
				true,
			);
			expect(shouldShowHoleCards('player', 'p2', 'p2', false, false)).toBe(
				true,
			);
		});

		it('should hide other players cards before showdown', () => {
			expect(shouldShowHoleCards('player', 'p1', 'p2', false, false)).toBe(
				false,
			);
			expect(shouldShowHoleCards('player', 'p2', 'p1', false, false)).toBe(
				false,
			);
		});
		...
		it('should show cards at showdown if not folded', () => {
			...
			expect(
				shouldShowHoleCards('spectator', undefined, 'p1', true, false),
			).toBe(true);
			expect(shouldShowHoleCards('replay', undefined, 'p1', true, false)).toBe(
				true,
			);
		});
		...
		it('should hide folded players cards even at showdown', () => {
			...
			expect(
				shouldShowHoleCards('spectator', undefined, 'p1', true, true),
			).toBe(false);
			expect(shouldShowHoleCards('replay', undefined, 'p1', true, true)).toBe(
				false,
			);
		});
	});
