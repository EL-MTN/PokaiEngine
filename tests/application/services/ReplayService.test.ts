...
import { IGameEvent, IReplay } from '@/infrastructure/persistence/models/Replay';
...
describe('ReplayService', () => {
  ...
  beforeEach(() => {
    ...
    (ReplayRepository as jest.Mock).mockImplementation(() => mockReplayRepository);
  ...
  describe('getReplay', () => {
    it('should retrieve replay by gameId', async () => {
      mockReplayRepository.findByGameId.mockResolvedValueOnce(mockReplay as IReplay);

      const result = await replayService.getReplay(mockGameId);

      expect(mockReplayRepository.findByGameId).toHaveBeenCalledWith(mockGameId);
      expect(result).toEqual(mockReplay);
    });
  ...
  describe('getRecentReplays', () => {
    it('should retrieve recent replays with default limit', async () => {
      const mockList = Array(10).fill({ gameId: 'game', startTime: new Date() });
  ...
  describe('addEvents', () => {
    ...
    it('should throw error if replay not found', async () => {
      ...
      await expect(
        replayService.addEvents(mockGameId, [])
      ).rejects.toThrow(`Replay not found for game ${mockGameId}`);
    });
  ...
  describe('getHandReplay', () => {
    ...
    it('should handle events without hand_complete', async () => {
      const incompleteEvents = mockEvents.filter((e) => e.type !== 'hand_complete');
      mockReplayRepository.getEventsByGameId.mockResolvedValueOnce(incompleteEvents);
