...
export class MongoMockUtils {
	...
	static setupModelMocks(
		...
		model.findOne.mockImplementation((filter) => {
			const query = this.createMockQuery();
			const result =
				defaultData.find((item) => this.matchesFilter(item, filter)) || null;
		...
		model.findById.mockImplementation((id) => {
			const query = this.createMockQuery();
			const result =
				defaultData.find((item) => (item as any)._id === id) || null;
			query.exec.mockResolvedValue(result);
			return query;
		});
		...
		model.findOneAndUpdate.mockImplementation((filter, update) => {
			const existing = defaultData.find((item) =>
		...
		model.deleteMany.mockResolvedValue({
			acknowledged: true,
			deletedCount: defaultData.length,
		});
		...
		return Object.keys(filter).every((key) => {
			...
			if (typeof filterValue === 'object' && filterValue !== null) {
				...
				if (filterValue.$in && !filterValue.$in.includes(itemValue))
					return false;
				if (filterValue.$nin && filterValue.$nin.includes(itemValue))
					return false;
				return true;
			}
	 ...
	 */
	static setupConnectionMocks(
		mockMongoose: jest.Mocked<typeof mongoose>,
	): void {
