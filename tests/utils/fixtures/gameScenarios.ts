...
export const GameScenarios = {
		...
		complex: (): GameState => {
			const players = [
				new PlayerBuilder()
					.withId('p1')
					.withChips(0)
					.withTotalBetThisHand(100)
					.asAllIn()
					.build(),
				new PlayerBuilder()
					.withId('p2')
					.withChips(0)
					.withTotalBetThisHand(300)
					.asAllIn()
					.build(),
				new PlayerBuilder()
					.withId('p3')
					.withChips(0)
					.withTotalBetThisHand(500)
					.asAllIn()
					.build(),
				new PlayerBuilder()
					.withId('p4')
	...
	specialCases: {
		onlyOneActivePlayer: (): GameState => {
			...
			return new GameStateBuilder().withPlayers(players).atPreflop().build();
		},
