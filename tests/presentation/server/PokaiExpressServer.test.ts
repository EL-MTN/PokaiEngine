...
describe('PokaiExpressServer', () => {
	...
	beforeEach(() => {
		...
		serverAny.gameController.getReplayFromAnySource = jest
			.fn()
			.mockImplementation(async (gameId) => {
				return mockReplayManager.getReplayFromAnySource(gameId);
			});

		serverAny.gameController.getReplayData = jest
			.fn()
			.mockImplementation((gameId) => {
				return mockReplayManager.getReplayData(gameId);
			});

		serverAny.gameController.getHandReplayData = jest
			.fn()
			.mockImplementation((gameId, handNumber) => {
				return mockReplayManager.getHandReplayData(gameId, handNumber);
			});

		serverAny.gameController.saveReplayToFile = jest
			.fn()
			.mockImplementation(async (gameId) => {
				return mockReplayManager.saveReplayToFile(gameId);
			});

		serverAny.gameController.exportReplay = jest
			.fn()
			.mockImplementation((gameId, format) => {
				return mockReplayManager.exportReplay(gameId, format);
			});

		serverAny.gameController.getReplaySystem = jest
			.fn()
			.mockReturnValue(mockReplaySystem);

		// Mock getGame to check deletedGames set
		serverAny.gameController.getGame = jest
			.fn()
			.mockImplementation((gameId) => {
				...
				if (
					gameId &&
					(gameId.includes('test') ||
						gameId.includes('game') ||
						gameId.includes('state') ||
						gameId.includes('start'))
				) {
					return {
		...
		serverAny.gameController.removeGame = jest
			.fn()
			.mockImplementation(async (gameId) => {
		...
		serverAny.gameController.createGame = jest
			.fn()
			.mockImplementation((gameId, config) => {
				createdGames.set(gameId, config);
				return originalCreateGame?.call(
					serverAny.gameController,
					gameId,
					config,
				);
			});
		...
		serverAny.botInterface.getGameInfo = jest
			.fn()
			.mockImplementation((gameId) => {
				if (deletedGames.has(gameId)) {
					return null;
		...
		serverAny.botInterface.findAvailableGames = jest
			.fn()
			.mockImplementation((maxPlayers) => {
	...
	describe('Game Management Routes', () => {
		...
		describe('GET /api/games/available', () => {
			beforeEach(async () => {
				...
				await request(app)
					.post('/api/games')
					.send({ gameId: 'game-2p', maxPlayers: 2 });
				await request(app)
					.post('/api/games')
					.send({ gameId: 'game-6p', maxPlayers: 6 });
				await request(app)
					.post('/api/games')
					.send({ gameId: 'game-9p', maxPlayers: 9 });
			});
		...
		describe('GET /api/games/:gameId', () => {
			it('should get specific game info', async () => {
				await request(app)
					.post('/api/games')
					.send({ gameId: 'info-test', maxPlayers: 4 });
		...
		describe('POST /api/games/:gameId/start', () => {
			it('should start a game', async () => {
				...
				const serverAny = server as any;
				serverAny.gameController.addPlayerToGame(
					'start-test',
					'p1',
					'Player1',
					1000,
	...
	describe('Bot Management Routes', () => {
		...
		beforeEach(() => {
			...
			jest
				.spyOn(BotAuthService, 'getInstance')
				.mockReturnValue(mockBotAuthService as any);
			// Reset all mocks
		...
		describe('POST /api/bots/register', () => {
			...
			it('should handle registration errors', async () => {
				mockBotAuthService.registerBot.mockRejectedValue(
					new Error('Bot already exists'),
				);
		...
		describe('GET /api/bots', () => {
			...
			it('should filter bots by status', async () => {
				...
				expect(mockBotAuthService.listBots).toHaveBeenCalledWith({
					status: 'active',
				});
			...
			it('should filter bots by developer', async () => {
				...
				await request(app).get('/api/bots?developer=Dev1').expect(200);
		...
		describe('POST /api/bots/:botId/regenerate-key', () => {
			...
			it('should handle errors', async () => {
				mockBotAuthService.regenerateApiKey.mockRejectedValue(
					new Error('Bot not found'),
				);
	...
	describe('Replay Routes', () => {
		...
		describe('GET /api/replays', () => {
			it('should list all replays', async () => {
				const response = await request(app).get('/api/replays').expect(200);
				...
				if (game && game.isGameRunning()) {
					...
					if (currentPlayerSeat !== undefined) {
						...
						const currentPlayer = gameState.players.find(
							(p: any) => p.position === currentPlayerSeat,
						);
						if (currentPlayer) {
							game.submitAction({
	...
	describe('Error Handling', () => {
		...
		it('should handle internal server errors gracefully', async () => {
			...
			const serverAny = server as any;
			serverAny.gameController.getOverallStats = jest
				.fn()
				.mockImplementation(() => {
