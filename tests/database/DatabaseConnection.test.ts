...
describe('DatabaseConnection', () => {
	...
	describe('healthCheck', () => {
		...
		it('should return false when ping fails', async () => {
			...
			mockMongoose.connect.mockResolvedValueOnce(mockMongoose as any);
			(
				mockMongoose.connection.db!.admin().ping as jest.Mock
	...
	describe('event handlers', () => {
		...
		it('should handle disconnection event', async () => {
			...
			const disconnectedHandler = (
				mockMongoose.connection.on as jest.Mock
			).mock.calls.find((call) => call[0] === 'disconnected')[1];
		...
		it('should handle reconnection event', async () => {
			...
			const reconnectedHandler = (
				mockMongoose.connection.on as jest.Mock
			).mock.calls.find((call) => call[0] === 'reconnected')[1];
