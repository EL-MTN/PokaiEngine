...
export class ReplayStorage {
	 ...
	 */
	async saveReplay(replayData: ReplayData): Promise<{
		fileSuccess: boolean;
		...
		// Save to file
		try {
			const filePath = this.saveReplayToFile(replayData);
			if (filePath) {
		...
		if (this.replayService) {
			try {
				await this.saveReplayToMongo(replayData);
	...
	private saveReplayToFile(replayData: ReplayData): string | null {
		if (!this.config.directory) return null;

		try {
			const filename = `${replayData.gameId}_${replayData.startTime.toISOString().replace(/[:.]/g, '-')}.json`;
			const filepath = path.join(this.config.directory, filename);
			
			const jsonData = JSON.stringify(replayData, null, 2);
			fs.writeFileSync(filepath, jsonData);
			
			replayLogger.info(`Saved replay to file: ${filepath}`);
			return filepath;
		} catch (error) {
			replayLogger.error('Failed to save replay to file:', error);
			return null;
