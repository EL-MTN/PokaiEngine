	...
	getGameStatistics(gameId: GameId):
		...
		for (const [playerId, stats] of playerStats) {
			if (stats.actionsCount > 0) {
				...
				stats.avgActionTime =
					playerActionTimes.reduce((sum, time) => sum + time, 0) /
					playerActionTimes.length;
			}
		}

		const gameLength = gameLog.endTime
			? gameLog.endTime - gameLog.startTime
			: Date.now() - gameLog.startTime;
