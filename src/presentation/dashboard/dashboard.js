...
class PokaiDashboard {
    ...
    setupEventListeners() {
        ...
        const closeBotDetailsBtn = document.getElementById('close-bot-details-modal');
        ...
        const botRegenerateKeyBtn = document.getElementById('bot-regenerate-key-btn');
    ...
    updateBotsTable() {
        ...
        const statusFilter = document.getElementById('bot-status-filter')?.value || '';
            ...
            ${bot.permissions ? `
                <div style="margin-top: 2rem;">
                    ...
                    <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                        ${Object.entries(bot.permissions).map(([perm, enabled]) => `
    ...
    async suspendBot() {
        ...
        try {
            ...
            if (result.success) {
                this.addLog('info', `Bot "${this.currentBotId}" suspended successfully`);
    ...
    async reactivateBot() {
        ...
        try {
            const response = await fetch(`/api/bots/${this.currentBotId}/reactivate`, {
                method: 'POST'
            });
            const result = await response.json();

            if (result.success) {
                this.addLog('info', `Bot "${this.currentBotId}" reactivated successfully`);
                document.getElementById('bot-details-modal').classList.remove('show');
                await this.loadBots();
            } else {
    ...
    async revokeBot() {
        ...
        if (!confirm(`Are you sure you want to revoke bot "${this.currentBotId}"? This action cannot be undone.`)) {
            return;
        }
    ...
    async regenerateBotKey() {
        ...
        if (!confirm(`Are you sure you want to regenerate the API key for bot "${this.currentBotId}"? The old key will stop working immediately.`)) {
            return;
        }

        try {
            const response = await fetch(`/api/bots/${this.currentBotId}/regenerate-key`, {
                method: 'POST'
            });
            const result = await response.json();

            if (result.success) {
                this.addLog('info', `API key regenerated for bot "${this.currentBotId}"`);
    ...
    async loadReplays() {
        try {
            ...
            if (result.success && result.data) {
                ...
                this.addLog('info', `Loaded ${result.data.length} replays (${stats.mongoReplays || 0} from DB, ${stats.fileReplays || 0} from files)`);
    ...
    async loadReplayMetadata() {
        ...
        for (const gameId of replayIds) {
            try {
                ...
                if (replay.handsPlayed === 0 && replay.path && !replay.path.startsWith('mongo://')) {
    ...
    updateReplaysDisplay() {
        ...
        replaysContainer.innerHTML = replays.map(replay => {
            const sourceIcon = replay.source === 'mongodb' 
            ...
            const gameTypeLabel = replay.gameType && replay.gameType !== 'unknown' ? ` (${replay.gameType})` : '';
    ...
    updateGameStateDisplay() {
        ...
        if (currentPhase) currentPhase.textContent = gameState.phase || gameState.currentPhase || 'Pre-Flop';
        if (currentPot) currentPot.textContent = gameState.potManager?.totalPot || gameState.potSize || 0;
    ...
    updatePlayersDisplay(players) {
        ...
        for (let i = 0; i < Math.max(6, players.length); i++) {
            ...
            if (player) {
                ...
                const isFolded = player.isFolded || (player.hasActed && player.lastAction === 'FOLD');
    ...
    stopSpectating() {
        if (this.currentSpectatingGame) {
            this.socket.emit('stopSpectating', { gameId: this.currentSpectatingGame });
            this.currentSpectatingGame = null;
    ...
    updateLiveGameDisplay(gameId, gameState) {
        ...
        document.getElementById('live-pot-amount').textContent = totalPot.toString();
        
        // Update game info
        document.getElementById('live-hand-number').textContent = gameState.handNumber || '0';
        document.getElementById('live-game-phase').textContent = gameState.currentPhase || '-';
        document.getElementById('live-blinds').textContent = `$${gameState.smallBlindAmount}/$${gameState.bigBlindAmount}`;
        
        // Update players
        this.updateLivePlayersDisplay(gameState.players, gameState.currentPlayerToAct);
    }
