...
describe('SocketHandler Comprehensive Tests', () => {
	...
	async function authenticateBot(socket: MockSocket, credentials: any): Promise<void> {
		socket.outgoing = [];
	...
	async function createAuthenticatedBot(socketId: string, credentials: any): Promise<MockSocket> {
	...
	describe('Basic Bot Flow', () => {
		it('lets two bots join and receive game state', async () => {
			...
			const bot1Success = bot1.outgoing.find(e => e.event === 'identificationSuccess');
			const bot2Success = bot2.outgoing.find(e => e.event === 'identificationSuccess');
	...
	describe('Error Handling', () => {
		...
		it('rejects identification to non-existent game', async () => {
			const bot1 = await createAuthenticatedBot(bot1Id, bot1Credentials);
			bot1.trigger('identify', { botName: 'Alpha', gameId: 'non-existent', chipStack: 1000 });
		...
		it('handles game at capacity', async () => {
			...
			const errorMsg = bot3.outgoing.find(e => e.event === 'identificationError');
			expect(errorMsg).toBeDefined();
	...
	describe('Game Events', () => {
		it('broadcasts events to all bots in game', async () => {
			...
			const possibleActionsMsg = actingBot.outgoing.find(e => e.event === 'possibleActions');
			if (possibleActionsMsg && possibleActionsMsg.data.possibleActions.length > 0) {
				...
				const bot1GameEvents = bot1.outgoing.filter(e => 
					e.event === 'gameEvent' || 
					e.event === 'gameState' || 
					e.event === 'actionSuccess'
				);
				const bot2GameEvents = bot2.outgoing.filter(e => 
					e.event === 'gameEvent' || 
					e.event === 'gameState'
				);
	...
	describe('Authentication', () => {
		it('requires authentication before actions', async () => {
			...
			const authRequired = socket.outgoing.find(e => e.event === 'authRequired');
