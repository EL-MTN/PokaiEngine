...
import { GameController, GameInfo } from '@/application/engine/GameController';
import { ActionType, GameConfig, Action, GameEvent, BotGameState, PossibleAction } from '@/domain/types';
...
describe('BotInterface', () => {
	...
	describe('joinGame', () => {
		...
		it('should throw error for non-existent game', () => {
			expect(() => {
				botInterface.joinGame('non-existent-game', playerId, botName, chipStack);
			}).toThrow();
		});
	...
	describe('getPossibleActions', () => {
		...
		it('should return empty array for non-existent player', () => {
			const possibleActions = botInterface.getPossibleActions(gameId, 'non-existent-player');
			expect(Array.isArray(possibleActions)).toBe(true);
	...
	describe('Integration Tests', () => {
		it('should handle complete game flow', () => {
			...
			expect(updatedGameState.players.find(p => p.id === playerId)?.isFolded).toBe(true);
		});
