import { IReplay, IGameEvent, IGameMetadata, IReplayAnalytics } from '@/infrastructure/persistence/models/Replay';
...
describe('Replay Model Interfaces', () => {
	describe('IGameMetadata validation', () => {
		...
		it('should validate time relationships', () => {
			...
			expect(metadata.gameDuration).toBe(metadata.gameEndTime - metadata.gameStartTime);
		});
	});
	...
	describe('IGameEvent validation', () => {
		...
		it('should support different event types', () => {
			const eventTypes = ['action', 'hand_started', 'hand_complete', 'game_started', 'game_complete'];
	...
	describe('IReplayAnalytics validation', () => {
		...
		it('should validate action distribution', () => {
			const analytics = createValidAnalytics();
			const totalActions = Object.values(analytics.actionDistribution).reduce((sum, count) => sum + count, 0);
			expect(totalActions).toBeGreaterThan(0);
		});

		it('should validate phase distribution', () => {
			const analytics = createValidAnalytics();
			const totalPhases = Object.values(analytics.phaseDistribution).reduce((sum, count) => sum + count, 0);
		...
		it('should validate game flow metrics', () => {
			...
			expect(analytics.gameFlow.longestHand).toBeGreaterThan(analytics.gameFlow.shortestHand);
	...
	describe('IReplay structure validation', () => {
		...
		it('should maintain data consistency', () => {
			...
			if (replay.handSummaries!.length > 1) {
				for (let i = 1; i < replay.handSummaries!.length; i++) {
					expect(replay.handSummaries![i].handNumber).toBeGreaterThan(replay.handSummaries![i-1].handNumber);
				}
			}
		});
		...
		it('should validate hand summary structure', () => {
			...
			expect(handSummary.endTimestamp).toBeGreaterThan(handSummary.startTimestamp);
			expect(handSummary.duration).toBe(handSummary.endTimestamp - handSummary.startTimestamp);
	...
	describe('data relationships', () => {
		it('should validate cross-references between collections', () => {
			...
			expect(metadata.winners.every(winner => metadata.playerNames[winner])).toBe(true);
		});
