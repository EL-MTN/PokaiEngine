...
describe('Replay Model', () => {
	...
	describe('schema validation', () => {
		...
		it('should save when endTime is before startTime (no validation)', async () => {
			const replayData = createValidReplayData();
			replayData.metadata!.gameStartTime = new Date('2023-01-01T12:00:00Z').getTime();
			replayData.metadata!.gameEndTime = new Date('2023-01-01T10:00:00Z').getTime(); // End before start allowed
			const replay = new Replay(replayData);

			// Should save successfully as there's no validation
			const savedReplay = await replay.save();
			expect(savedReplay.metadata.gameStartTime).toBeGreaterThan(savedReplay.metadata.gameEndTime);
		});
	...
	describe('event validation', () => {
		...
		it('should allow valid action types', async () => {
			const validActionTypes = ['fold', 'check', 'call', 'bet', 'raise', 'all_in'];
				...
				replayData.events = [{
					...
					data: {
						action: {
							...
							amount: actionType === 'bet' || actionType === 'raise' || actionType === 'all_in' ? 100 : undefined
						}
					},
				}];
				const replay = new Replay(replayData);
	...
	describe('data consistency', () => {
		it('should maintain referential integrity in events', async () => {
			...
			expect(savedReplay.events[0].timestamp).toBeLessThan(savedReplay.events[1].timestamp);
		});
