import { ReplayRepository, ReplaySearchFilters } from '@/infrastructure/persistence/repositories/ReplayRepository';
...
describe('ReplayRepository', () => {
	...
	const createMockGameEvent = (overrides?: Partial<IGameEvent>): IGameEvent => ({
		type: 'action',
		timestamp: Date.now(),
		data: {
			action: {
	...
	describe('create', () => {
		...
		it('should throw error when creation fails', async () => {
			...
			await expect(repository.create(replayData)).rejects.toThrow('Failed to create replay: Database error');
		});
	});
	...
	describe('findById', () => {
		...
		it('should throw error when database query fails', async () => {
			const mockId = new Types.ObjectId().toString();
			mockReplay.findById = jest.fn().mockRejectedValue(new Error('Database error'));

			await expect(repository.findById(mockId)).rejects.toThrow('Failed to find replay by ID: Database error');
		});
	});
	...
	describe('findByGameId', () => {
		...
		it('should throw error when query fails', async () => {
			const gameId = 'test-game-123';
			mockReplay.findOne = jest.fn().mockRejectedValue(new Error('Database error'));

			await expect(repository.findByGameId(gameId)).rejects.toThrow('Failed to find replay by game ID: Database error');
		});
	});
	...
	describe('findAll', () => {
		...
		it('should throw error when query fails', async () => {
			...
			await expect(repository.findAll()).rejects.toThrow('Failed to find replays: Database error');
		});
	});
	...
	describe('getEventsByGameId', () => {
		...
		it('should filter events by hand number', async () => {
			...
			const result = await repository.getEventsByGameId(gameId, { handNumber: 1 });
	...
	describe('exists', () => {
		...
		it('should throw error when query fails', async () => {
			const gameId = 'test-game-123';
			mockReplay.countDocuments = jest.fn().mockRejectedValue(new Error('Database error'));

			await expect(repository.exists(gameId)).rejects.toThrow('Failed to check if replay exists: Database error');
		});
	});
	...
	describe('delete', () => {
		...
		it('should throw error when deletion fails', async () => {
			const gameId = 'test-game-123';
			mockReplay.deleteOne = jest.fn().mockRejectedValue(new Error('Database error'));

			await expect(repository.delete(gameId)).rejects.toThrow('Failed to delete replay: Database error');
		});
	});
	...
	describe('updateAnalytics', () => {
		it('should update analytics successfully', async () => {
			...
			mockReplay.findOneAndUpdate = jest.fn().mockResolvedValue(mockUpdatedReplay);
		...
		it('should throw error when update fails', async () => {
			...
			mockReplay.findOneAndUpdate = jest.fn().mockRejectedValue(new Error('Database error'));

			await expect(repository.updateAnalytics(gameId, analytics)).rejects.toThrow('Failed to update analytics: Database error');
		});
	});
	...
	describe('getStorageStats', () => {
		...
		it('should throw error when aggregation fails', async () => {
			mockReplay.aggregate = jest.fn().mockRejectedValue(new Error('Database error'));

			await expect(repository.getStorageStats()).rejects.toThrow('Failed to get storage stats: Database error');
		});
	});
});