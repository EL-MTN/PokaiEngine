import { GameLogger, GameLog } from '@logging/GameLogger';
import { GameState, PlayerInfo, GameEvent, Action, GamePhase, Pot, ActionType } from '@types';
...
describe('GameLogger', () => {
	...
	it('should log a game event', () => {
		gameLogger.startGame('test-game', initialGameState, playerNames);
		const event: GameEvent = { type: 'hand_started', handNumber: 1, timestamp: Date.now() };
	...
	it('should throw an error when ending a non-existent game', () => {
		...
		expect(() => gameLogger.endGame('non-existent-game', finalGameState)).toThrow(
			'No log found for game non-existent-game'
		);
	});
	...
	it('should return undefined for hand summary if start or end event is missing', () => {
		...
		const endEvent: GameEvent = { type: 'hand_complete', handNumber: 1, timestamp: Date.now() };
