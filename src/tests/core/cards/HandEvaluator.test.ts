...
describe('HandEvaluator', () => {
  ...
  describe('evaluateFiveCardHand', () => {
    ...
    it('should correctly identify a Royal Flush', () => {
      ...
      expect(result.value).toBeGreaterThan(HandRank.StraightFlush * 10000000000);
    });
    ...
    it('should correctly identify an Ace-low Straight Flush (Steel Wheel)', () => {
      ...
      expect(result.cards.map(c => c.rank).sort((a,b) => b-a)).toEqual([14, 5, 4, 3, 2]);
    });
    ...
    it('should throw an error if not exactly 5 cards are provided', () => {
      ...
      expect(() => HandEvaluator.evaluateFiveCardHand(hand)).toThrow('Hand must contain exactly 5 cards');
    });
  });
  ...
  describe('evaluateHand (7-card evaluation)', () => {
    ...
    it('should use one hole card to make a better straight', () => {
        ...
        const result = HandEvaluator.evaluateHand(holeCards, communityCards);
        expect(result.rank).toBe(HandRank.Straight);
        expect(result.cards.map(c => c.toString()).sort()).toEqual(['3H', '4D', '5C', '6S', '7S'].sort());
    });
    ...
    it('should throw an error if fewer than 5 total cards are provided', () => {
        ...
        expect(() => HandEvaluator.evaluateHand(holeCards, communityCards)).toThrow('Need at least 5 cards to evaluate hand');
    });
  });

  describe('Hand Comparison (Edge Cases)', () => {
    it('should correctly compare two different flushes', () => {
      const flush1 = HandEvaluator.evaluateFiveCardHand(createHand(['AS', 'KS', 'QS', 'JS', '8S']));
      const flush2 = HandEvaluator.evaluateFiveCardHand(createHand(['AD', 'KD', 'QD', 'JD', '7D']));
    ...
    it('should correctly compare two flushes with same high cards but different kickers', () => {
        const flush1 = HandEvaluator.evaluateFiveCardHand(createHand(['AS', 'KS', 'QS', 'JS', '8S']));
        const flush2 = HandEvaluator.evaluateFiveCardHand(createHand(['AS', 'KS', 'QS', 'JS', '7S']));
        expect(HandEvaluator.compareHands(flush1, flush2)).toBe(1);
    });

    it('should correctly compare two different straights', () => {
      const straight1 = HandEvaluator.evaluateFiveCardHand(createHand(['TH', '9S', '8D', '7C', '6H']));
      const straight2 = HandEvaluator.evaluateFiveCardHand(createHand(['9H', '8S', '7D', '6C', '5H']));
      expect(HandEvaluator.compareHands(straight1, straight2)).toBe(1);
    });

    it('should correctly compare two full houses (higher trips win)', () => {
      const fullHouse1 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', 'AD', '2C', '2H']));
      const fullHouse2 = HandEvaluator.evaluateFiveCardHand(createHand(['KH', 'KS', 'KD', 'QC', 'QH']));
    ...
    it('should correctly compare two full houses (higher pair wins if trips are same)', () => {
        ...
        const fullHouse1 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', 'AD', 'KC', 'KH']));
        const fullHouse2 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', 'AD', 'QC', 'QH']));
        expect(HandEvaluator.compareHands(fullHouse1, fullHouse2)).toBe(1);
    });

    it('should correctly compare two Two-Pairs (higher top pair wins)', () => {
      const twoPair1 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', '2D', '2C', 'KH']));
      const twoPair2 = HandEvaluator.evaluateFiveCardHand(createHand(['KH', 'KS', 'QD', 'QC', 'JH']));
    ...
    it('should correctly compare two Two-Pairs (higher bottom pair wins)', () => {
      const twoPair1 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', '3D', '3C', 'KH']));
      const twoPair2 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', '2D', '2C', 'KH']));
      expect(HandEvaluator.compareHands(twoPair1, twoPair2)).toBe(1);
    });

    it('should correctly compare two Two-Pairs (kicker wins)', () => {
      const twoPair1 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', '2D', '2C', 'KH']));
      const twoPair2 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', '2D', '2C', 'QH']));
      expect(HandEvaluator.compareHands(twoPair1, twoPair2)).toBe(1);
    });

    it('should correctly compare two pairs (kicker wins)', () => {
      const pair1 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', 'KD', 'JC', 'TH']));
      const pair2 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', 'KD', 'JC', '9H']));
      expect(HandEvaluator.compareHands(pair1, pair2)).toBe(1);
    });

    it('should result in a tie for identical hands', () => {
        const hand1 = HandEvaluator.evaluateFiveCardHand(createHand(['AH', 'AS', 'KD', 'JC', 'TH']));
        const hand2 = HandEvaluator.evaluateFiveCardHand(createHand(['AD', 'AC', 'KH', 'JS', 'TC']));
