...
describe('Deck', () => {
  ...
  describe('dealHoleCards', () => {
    ...
    it('should throw an error if not enough cards to deal hole cards', () => {
      expect(() => deck.dealHoleCards(27)).toThrow('Cannot deal hole cards to 27 players, insufficient cards');
    });
  });
  ...
  describe('burnCard', () => {
    it('should throw an error when burning from an empty deck', () => {
        deck.dealCards(52);
        expect(() => deck['burnCard']()).toThrow('Cannot burn card from empty deck');
    });
  });
  ...
  describe('validate', () => {
    it('should return false for a deck with duplicate cards', () => {
      const cards = [new Card(Suit.Hearts, Rank.Ace), new Card(Suit.Hearts, Rank.Ace)];
      const customDeck = Deck.fromCards(cards);
